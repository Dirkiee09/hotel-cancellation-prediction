...
**Method.** Seven regressors fit on the chronological train split, tuned on
the validation set, evaluated on the held-out test set. The Portugal
champion (selected by val RMSE) is **Gradient Boosting**. RMSE in EUR;
